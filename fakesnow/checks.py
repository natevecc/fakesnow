...
def _missing_qualifiers(node: exp.Table) -> tuple[bool, bool]:
    ...
    # A table on the right-hand side of a JOIN is a query source, not a DDL
    # target. Its parent.kind is the join type ("INNER", "LEFT", ...) which
    # collides with the DDL-kind switch below. Apply the default qualifier
    # rule directly so the assertion isn't tripped.
