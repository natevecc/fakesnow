...
def test_alter_table(dcur: snowflake.connector.cursor.SnowflakeCursor):
    ...
    dcur.execute("alter table if exists table1 add column if not exists col3 int, col4 varchar(50)")
    dcur.execute("select id, name, col1, col2, col3, col4 from table1")
...
def test_create_database_respects_if_not_exists() -> None:
    with tempfile.TemporaryDirectory(prefix="fakesnow-test") as db_path, fakesnow.patch(db_path=db_path):
        ...
        with pytest.raises(ProgrammingError, match=r'Cannot attach "DB2" - the database file .* is already attached'):
